  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex gap-2 text-sm text-[var(--txt-muted)]">
        ...
        >
          {categories.find((cat) => cat.id === currentThread.categoryId)?.name ||
            "Category"}
        </Link>
      </div>
        ...
        <div className="p-6">
          ...
          <div className="flex flex-wrap items-center gap-x-4 text-sm text-[var(--txt-muted)]">
            ...
            <div className="flex items-center">
              ...
              <span>Posted {formatRelativeTime(currentThread.createdAt)}</span>
            </div>
          ...
          <div className="space-y-4">
                  ...
                  {currentUser?.uid === reply.authorId && (
                    <div className="flex gap-2">
                      ...
                      <button
                        ...
                        onClick={() => handleReplyDelete(reply.id, currentThread.id)}
                        disabled={deletingReplyId === reply.id}
                        aria-disabled={deletingReplyId === reply.id}
                      >
            ...
            {pagination.replies.hasMore && (
              <div className="flex justify-center">
                ...
                >
                  {loadingMore ? <Spinner size={12} /> : "Load More Replies"}
                </button>
              </div>
            )}
          </div>
