//TODO: split the file into smaller components later on
...
export function ModelUpload() {
  ...
  const hasPendingVerification = Boolean(
    privateProfile?.stripeConnectId && !privateProfile.stripeConnectStatus?.isFullyActive
  );
  ...
  useEffect(() => {
    ...
    if (isReturnFromStripe) {
      ...
      paymentService
        ...
        .then((status) => {
          if (status.isFullyActive) {
            console.log("✅ [URL Change] Connect account is now active after return");
            notification.success(
              "Welcome Back! 🎉",
        ...
        .catch((error) => {
          console.warn("⚠️ [URL Change] Failed to check Connect status:", error);
  ...
  const handleSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    if (modelData.isPaid && modelData.price && modelData.price > 0) {
      ...
      if (!privateProfile?.stripeConnectId) {
        console.log("❌ [Upload] No Connect account found - verification required");
      ...
      if (privateProfile?.stripeConnectStatus?.isFullyActive) {
        console.log("✅ [Upload] Using cached status - Connect account is fully active");
      } else {
        ...
        try {
          ...
          if (!status.isFullyActive) {
            ...
            if (status.requirementsDue.length > 0) {
              ...
              setError("Seller verification required to upload paid models.");
            }
  ...
  return (
    <div
      className={`flex items-center space-x-3 ${
        isActive ? "text-accent" : isCompleted ? "text-success" : "text-txt-secondary"
      }`}
    >
      <div
