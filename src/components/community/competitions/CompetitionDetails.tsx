...
import { mockEntries } from "./mockCompetitions";
...
export const CompetitionDetails = () => {
    ...
    return (
        <div className="max-w-4xl mx-auto py-8">
            ...
            <h1 className="text-3xl font-bold mb-2 text-[var(--txt-primary)]">
                {event.title}
            </h1>
            <div className="mb-2 text-[var(--txt-secondary)]">
                {event.startDate} - {event.endDate}
            </div>
            <div className="mb-4 text-[var(--txt-muted)]">
                Status: <span className="font-semibold">{event.status}</span>
            </div>
            <div className="mb-4 text-lg text-[var(--txt-primary)]">
                {event.description}
            </div>
            ...
            {event.rules && (
                <div className="mb-4">
                    ...
                    <div className="text-[var(--txt-secondary)] whitespace-pre-line">
                        {event.rules}
                    </div>
                </div>
            )}
            ...
            {(event.type === "meetup" || event.type === "webinar") &&
                event.status === "upcoming" && (
                    <div className="mb-8">
                        <button className="inline-block px-6 py-2 rounded bg-[var(--accent)] text-[var(--txt-highlight)] font-semibold hover:bg-[var(--accent-hover)]">
                            RSVP
                        </button>
                    </div>
                )}
