    ...
    <div className="relative" ref={dropdownRef}>
      <button
        ...
        className="
                    ...
                    bg-surface-card text-txt-primary text-sm
      ...
      {isOpen && (
        <div className="absolute top-full left-0 mt-1 w-80 bg-surface-card border border-br-secondary rounded-lg shadow-lg z-50 max-h-80 overflow-y-auto">
          <div className="p-3">
            <div className="text-sm font-medium text-txt-primary mb-3">
              Select Categories
            </div>
            {categories.map((category) => (
              <label
                key={category.id}
                className="flex items-center px-3 py-3 hover:bg-section rounded cursor-pointer"
              >
                <input
                  type="checkbox"
