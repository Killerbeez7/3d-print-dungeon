    ...
    <div className=" text-[var(--txt-primary)] rounded-lg shadow p-6">
      ...
      <div className="mb-6 flex items-center gap-4">
        ...
        <div>
          ...
          <p className="text-[var(--txt-secondary)]">Here&apos;s your forum activity at a glance.</p>
        </div>
      </div>

      {/* Quick Actions */}
      <div className="flex gap-4 mb-6">
        <Link to="/forum/new-thread" className="cta-button px-4 py-2">New Thread</Link>
        <Link to="/profile" className="secondary-button px-4 py-2">My Profile</Link>
      </div>
      ...
      <div className="mb-6">
        ...
        {loading ? (
          ...
          <div className="text-[var(--txt-muted)]">You haven&apos;t started any threads yet.</div>
        ) : (
          <ul className="space-y-2">
            {userThreads.map((thread) => (
              <li key={thread.id}>
                <Link to={`/forum/thread/${thread.id}`} className="font-medium hover:text-[var(--accent)]">
                  {thread.title}
                </Link>
        ...
        <Link to="/forum/my-threads" className="text-[var(--accent)] text-sm">View all my threads</Link>
      </div>
      ...
      <div>
        ...
        {loading ? (
          ...
          <div className="text-[var(--txt-muted)]">You haven&apos;t replied to any threads yet.</div>
        ) : (
          <ul className="space-y-2">
            {userReplies.map((reply) => (
              <li key={reply.id}>
                <span className="text-[var(--txt-muted)]">On </span>
                <Link to={`/forum/thread/${reply.threadId}`} className="font-medium hover:text-[var(--accent)]">
        ...
        <Link to="/forum/my-threads" className="text-[var(--accent)] text-sm">View all my replies</Link>
      </div>
