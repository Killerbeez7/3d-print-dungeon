    ...
    <div className="w-full max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
              ...
              className={`flex-1 px-2 sm:px-4 py-2 rounded font-semibold transition text-center text-sm sm:text-base hover:cursor-pointer
                    ${
                      type === key
                        ...
                        : "bg-surface-card text-txt-primary hover:bg-muted"
                    }`}
            >
              <span className="hidden sm:inline">{label}</span>
              <span className="sm:hidden">
                {mobileTypeLabels[key as EventType | "all"]}
              </span>
            </button>
          ))}
        </div>
      ...
      {showCreate && (
        <div className="fixed inset-0 z-50 flex items-center justify-center">
          ...
          {/* Modal content */}
          <div className="relative bg-section border border-br-secondary rounded-2xl shadow-2xl p-8 max-w-2xl w-full mx-4 transform transition-all duration-300 ease-out">
            {/* Close button */}
            <button
              onClick={() => setShowCreate(false)}
              className="absolute top-4 right-4 p-2 rounded-full hover:bg-muted text-txt-muted hover:text-txt-primary transition-colors duration-200"
              aria-label="Close"
            >
              <svg
                className="w-6 h-6"
