                            ...
                            <Draggable key={render.id} draggableId={render.id} index={index}>
                                {(provided, snapshot) => (
                                    <div
                                        ...
                                        className={`group relative flex-shrink-0 w-28 h-20 rounded-lg overflow-hidden bg-bg-surface cursor-pointer transition-all duration-200
                                            ${snapshot.isDragging ? 'ring-2 ring-accent scale-105 shadow-lg' : ''}
                                            ${render.id === primaryId ? 'ring-2 ring-accent' : 'hover:ring-2 hover:ring-accent'}`
                                        }
                                    >
                                        <img src={render.preview} alt={`Render ${index + 1}`} className="w-full h-full object-cover" />
                                        <div className="absolute inset-0 bg-bg-reverse opacity-0 group-hover:opacity-20 transition-opacity" />
                                        <button
                                            onClick={(e) => {
                                                e.stopPropagation();
                                                onRemove(render.id);
                                            }}
                                            className="contrast-button absolute top-1 right-1 w-6 h-6 flex items-center justify-center opacity-0 group-hover:opacity-100 transition-all duration-150"
