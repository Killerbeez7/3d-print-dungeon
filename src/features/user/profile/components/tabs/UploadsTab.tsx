...
export const UploadsTab = ({ userId }: UploadsTabProps) => {
  ...
  useEffect(() => {
    ...
    filtered.sort((a, b) => {
      switch (sortBy) {
        case "newest":
          return (
            getDateFromTimestamp(b.createdAt).getTime() -
            getDateFromTimestamp(a.createdAt).getTime()
          );
        case "oldest":
          return (
            getDateFromTimestamp(a.createdAt).getTime() -
  ...
  return (
    <div className="space-y-6">
      ...
      {/* Filters and Search */}
      <div className="bg-section rounded-lg p-4 space-y-4">
        {/* Search */}
        <div className="relative">
          ...
          <input
            ...
            className="w-full pl-10 pr-4 py-2 bg-surface-card border border-br-primary rounded-lg text-txt-primary focus:border-accent focus:outline-none"
          />
        </div>
        ...
        <div className="flex flex-col md:flex-row gap-4">
          ...
          <div className="flex-1">
            ...
            <select
              ...
              className="w-full px-3 py-2 bg-surface-card border border-br-primary rounded-lg text-txt-primary focus:border-accent focus:outline-none"
            >
      ...
      {filteredArtworks.length === 0 ? (
        <div className="text-center py-12">
          ...
          <h4 className="text-lg font-semibold text-txt-primary mb-2">
            No Uploads Found
          </h4>
          ...
          {!searchTerm && selectedCategory === "all" && (
            <Button leftIcon={<i className="fas fa-plus mr-2" />}>Upload Model</Button>
          )}
        </div>
      ) : (
        <div
        ...
        >
          {filteredArtworks.map((model) => (
            <article
              ...
              className={`bg-section border border-br-primary rounded-lg overflow-hidden hover:shadow-lg transition-all duration-200 cursor-pointer ${
                viewMode === "list" ? "flex" : ""
              }`}
            >
              <div
                className={viewMode === "list" ? "w-32 h-32 flex-shrink-0" : "relative"}
              >
                <LazyImage
                  src={
              ...
              <div className={`p-4 ${viewMode === "list" ? "flex-1" : ""}`}>
                ...
                {model.tags && model.tags.length > 0 && viewMode === "list" && (
                  <div className="flex flex-wrap gap-1 mt-3">
                    ...
                    {model.tags.length > 3 && (
                      <span className="px-2 py-1 bg-page text-txt-secondary text-xs rounded-full">
                        +{model.tags.length - 3}
                      </span>
                    )}
                  </div>
