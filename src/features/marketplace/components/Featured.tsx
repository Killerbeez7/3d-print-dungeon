// TODO: load models with TanStack Query when this page is implemented.
...
export const Featured: FC<FeaturedProps> = ({ previewCount }) => {
  ...
  //             featuredCategories.some(category => model.categoryIds?.includes(category))
  //         );
  ...
  const modelsToShow = previewCount
    ? featuredModels.slice(0, previewCount)
    : featuredModels;
            ...
            <Link key={model.id} to={`/model/${model.id}`}>
              <article className="relative bg-surface-card rounded-md overflow-hidden shadow-sm hover:shadow-md transition-shadow">
                <div className="relative w-full aspect-square">
                  <LazyImage
                    src={
                      getThumbnailUrl(model.renderPrimaryUrl, "MEDIUM") ||
