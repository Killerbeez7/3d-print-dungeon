...
export const ExpandedBlogCard = ({ post, onClose }: ExpandedBlogCardProps) => {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center">
      ...
      <div className="relative bg-section border border-br-secondary rounded-2xl shadow-2xl max-w-3xl w-full mx-4 max-h-[80vh] overflow-hidden">
        {/* Close button */}
        <button
          onClick={onClose}
          ...
          <div className="p-6">
            ...
            <div className="flex flex-wrap gap-2 mb-4">
              {post.tags.map((tag) => (
                <span
                  key={tag}
                  className="px-3 py-1 bg-muted text-txt-secondary text-sm rounded-full"
                >
                  {tag}
                </span>
              ))}
            ...
            {/* Title */}
            <h1 className="text-2xl font-bold text-txt-primary mb-4">{post.title}</h1>
            ...
            <div className="flex items-center justify-between mb-4 pb-4 border-b border-br-secondary">
              <div className="flex items-center space-x-3">
                ...
                <div>
                  ...
                  <p className="text-sm text-txt-muted">{formatDate(post.publishedAt)}</p>
                </div>
              </div>
            ...
            <div className="mb-6">
              <p className="text-txt-secondary leading-relaxed mb-4">{post.content}</p>
            ...
            <div className="flex justify-center space-x-4 pt-4 border-t border-br-secondary">
              ...
              <button className="px-6 py-2 bg-muted hover:bg-surface-card text-txt-primary rounded-lg font-semibold transition-colors duration-200">
                Share Article
              </button>
            </div>
          </div>
