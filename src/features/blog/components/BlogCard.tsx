...
export const BlogCard = ({ post, isExpanded = false, onToggle }: BlogCardProps) => {
  ...
  return (
    <>
      ...
      {isExpanded && (
        <div className="bg-section border border-br-secondary rounded-xl overflow-hidden shadow-sm animate-pulse">
      ...
      {/* Normal Card */}
      <article
        className={`bg-section border border-br-secondary rounded-xl overflow-hidden shadow-sm hover:shadow-lg transition-all duration-300 cursor-pointer ${
          isExpanded ? "hidden" : ""
        }`}
        ...
        <div className="p-6">
          ...
          <div className="flex flex-wrap gap-2 mb-3">
            {post.tags.slice(0, 3).map((tag) => (
              <span
                key={tag}
                className="px-2 py-1 bg-muted text-txt-secondary text-xs rounded-full"
              >
                {tag}
              </span>
            ))}
          ...
          {/* Excerpt */}
          <p className="text-txt-secondary mb-4 line-clamp-3">{post.excerpt}</p>
          ...
          <div className="flex items-center justify-between mt-4 pt-4 border-t border-br-secondary">
            <div className="flex items-center space-x-3">
              ...
              />
              <div>
                <p className="text-sm font-medium text-txt-primary">{post.author.name}</p>
                <p className="text-xs text-txt-muted">{formatDate(post.publishedAt)}</p>
              </div>
            </div>
